...
def extract_aligned_metadata(lm):
...
def apply_alignment(img, quad, qsize, output_size=1024):
    transform_size = 4096
    enable_padding = True
...
def align_face(
    ...
    for num_img, lm in enumerate(lms):
        quad, qsize = extract_aligned_metadata(lm)
        ...
        img = apply_alignment(img, quad, qsize, output_size)
