...
import torch
from live_portrait.models.utils import Hourglass, make_coordinate_grid, kp2gaussian
...
class DenseMotionNetwork(nn.Module):
    def __init__(self,
        ...
        self.compress = nn.Conv3d(
            feature_channel, compress, kernel_size=1)  # 0.8G
    ...
    def create_deformed_feature(self, feature, sparse_motions):
        ...
        feature_repeat = feature.unsqueeze(1).unsqueeze(1).repeat(
            1, self.num_kp+1, 1, 1, 1, 1, 1)  # (bs, num_kp+1, 1, c, d, h, w)
        # (bs*(num_kp+1), c, d, h, w)
    ...
    def create_heatmap_representations(self, feature, kp_driving, kp_source):
        ...
        # adding background feature
        zeros = torch.zeros(heatmap.shape[0], 1,
                            spatial_size[0],
                            spatial_size[1],
                            spatial_size[2]).type(
            heatmap.dtype).to(heatmap.device)
    ...
    def forward(self, feature, kp_driving, kp_source):
        ...
        sparse_motion = self.create_sparse_motions(
            feature, kp_driving, kp_source)  # (bs, 1+num_kp, d, h, w, 3)
        deformed_feature = self.create_deformed_feature(
            feature, sparse_motion)  # (bs, 1+num_kp, c=4, d=16, h=64, w=64)

        # 2. (bs, 1+num_kp, d, h, w)
        heatmap = self.create_heatmap_representations(
            deformed_feature, kp_driving, kp_source)  # (bs, 1+num_kp, 1, d, h, w)

        # (bs, 1+num_kp, c=5, d=16, h=64, w=64)
        input = torch.cat([heatmap, deformed_feature], dim=2)
        # (bs, (1+num_kp)*c=105, d=16, h=64, w=64)
        ...
        if self.flag_estimate_occlusion_map:
            ...
            occlusion_map = torch.sigmoid(
                self.occlusion(prediction_reshape))  # Bx1x64x64
