...
class Pose2VideoPipeline(DiffusionPipeline):
        ...
        if isinstance(condition, torch.Tensor):
            ...
            for i in condition:
                if isinstance(i, torch.Tensor):
                    ...
                    stacks.append(self.images2latents(i, dtype).cuda())
            condition = torch.cat(stacks, axis=0)
        if do_classifier_free_guidance:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        noise_aug_strength = torch.tensor(
            noise_aug_strength, dtype=torch.float32).unsqueeze(0)
        noise_aug_strength = noise_aug_strength.to(device)
        if (x is not None) and (y is not None):
            x = x.to(device)
            y = y.to(device)
            add_time_ids = self._get_add_time_ids(
                noise_aug_strength, x, y, do_classifier_free_guidance)
...
class Pose2VideoPipeline2(DiffusionPipeline):
    _optional_components = []
...
class Pose2VideoPipeline_controlnet(DiffusionPipeline):
    _optional_components = []
    ...
    def __call__(
        ...
        **kwargs,
        ...
        controls = self.prepare_condition(
            condition=controlnet_condition,
            device=device,
            dtype=controlnet.dtype,
            do_classifier_free_guidance=do_classifier_free_guidance,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    control1 = torch.cat(
                        [controls[:21, :, :][c, :, :, :] for c in context])
                    control2 = torch.cat(
                        [controls[21:, :, :][c, :, :, :] for c in context])
                    control = torch.cat([control1, control2], axis=0)
...
class Pose2VideoPipeline_controlnet_with_temporal(DiffusionPipeline):
    _optional_components = []
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                global_context = []
                # control = rearrange(
                #     control, "(b f) c h w -> b f c h w", f = video_length
                # )
                for i in range(num_context_batches):
                    global_context.append(
                        context_queue[
                ...
                for context in global_context:
                    ...
                    video_length2 = noisy_latents_control.shape[1]

                    if len(control.shape) == 4:
                        control = rearrange(
                            control, "(b f) c h w -> b f c h w", f=video_length2
                        )
...
class Pose2VideoPipeline_stable_hair(DiffusionPipeline):
    _optional_components = []
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    # ).repeat(2 if do_classifier_free_guidance else 1, 1, 1, 1, 1)
                    # Add_time_ids = torch.cat(add_time_ids[c,:].unsqueeze(0))
                    # add_time_ids = self._get_add_time_ids(noise_aug_strength, x[context[0]], y[context[0]], do_classifier_free_guidance)
                    noisy_latents_control = rearrange(
                        latent_model_input, "b c f h w -> (b f) c h w"
                    )
                    control = control.reshape(2, 21, 4, 64, 64)
                    # print(context)
                    # print(noisy_latents_control.shape)
                    # print(control.shape)

                    control_input = (
                        torch.cat([control[:, [c], ] for c in context], axis=2)
                        .to(device)
                    )

                    control_input = control_input.reshape(-1, 4, 64, 64)
                    control = control.reshape(-1, 4, 64, 64)
